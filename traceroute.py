# Name: Casey Levy
# CS 372 - Traceroute
# Description: Augmenting a raw socket to enhance ping accuracy and create
#              traceroute functionality.
# Sources Cited:
# "Computer Networking: A Top-Down Approach", Kurose & Ross, 7th Edition
# https://docs.python.org/3.8/howto/sockets.html
# All other sources used were given within the skelton code
...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            if icmpReplyPacket.getIcmpSequenceNumber() != self.getPacketSequenceNumber():
                icmpReplyPacket.setIcmpNumValid(False)
                is_valid = False

            if icmpReplyPacket.getIcmpIdentifier() != self.getPacketIdentifier():
                icmpReplyPacket.setIcmpIDValid(False)
                is_valid = False

            if icmpReplyPacket.getIcmpData() != self.getDataRaw():
                icmpReplyPacket.setIcmpDataValid(True)
                is_valid = False
        ...
        def sendEchoRequest(self):
            ...
            try:
                ...
                # Need to increase the amount of sent packets
                IcmpHelperLibrary.pack_data[0] += 1
                timeLeft = 30
                ...
                IcmpHelperLibrary.RTT_amt.append(timeReceived - pingStartTime)
                if timeLeft <= 0:
                    ...
                    # Need to increase number of received packets
                    IcmpHelperLibrary.pack_data[1] += 1
    ...
    class IcmpPacket_EchoReply:
        ...
        # Getting and returning status of the ICMP seq number
        def getIcmpNumValid(self):
            return self.IcmpNum_Valid

        # Getting and returning status of ICMP raw data
        def getIcmpDataValid(self):
            return self.IcmpData_Valid

        # Getting and returning status of ICMP ID
        def getIcmpIDValid(self):
            return self.IcmpID_Valid
        ...
        def setIcmpNumValid(self, booleanValue):
            self.IcmpNum_Valid = booleanValue

        def setIcmpIDValid(self, booleanValue):
            self.IcmpID_Valid = booleanValue

        def setIcmpDataValid(self, booleanValue):
            self.IcmpData_Valid = booleanValue
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            ...
            print("  TTL=%d    RTT=%.0f ms    Type=%d    Code=%d        Identifier=%d    Sequence Number=%d    %s   Valid=%s" %
                  (
                      ttl,
            ...
            if not self.isValidResponse():
                ...
                if not self.getIcmpDataValid():
                    print("Mismatched Data: Data Expected =", IcmpPacket.getDataRaw(), "   Data Received =", self.getIcmpData()) 

                if not self.getIcmpNumValid():
                     print("Mismatched Seq Number: Number Expected =", IcmpPacket.getPacketSequenceNumber(), "   Number Received =", self.getIcmpSequenceNumber()) 
                
                if not self.getIcmpIDValid():
                    print("Mismatched ID: ID Expected =", IcmpPacket.getPacketIdentifier(), "   ID Received =", self.getIcmpIdentifier()) 
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(4):
            ...
            # Printing errors and codes
            if(icmpPacket.getIcmpType() != 0) and (icmpPacket.getIcmpType() in Icmp_errors):
                print("ERROR: CODE " + icmpPacket.getIcmpCode() + " --- TYPE " + icmpPacket.getIcmpType() + 
                        ": " + Icmp_errors[icmpPacket.getIcmpCode()][icmpPacket.getIcmpType()])

        # Showing round-trip time (RTT) info, packet loss, and packet loss rate
        avgRTT = sum(self.RTT_amt) / len(self.RTT_amt) * 1000
        print(self.RTT_amt)
        print("\nPing Information For " + host + ":")
        print("\n --- RTT INFORMATION --- ")
        print("MIN=%d ms, MAX=%d ms, AVG=%d ms" % (min(self.RTT_amt) * 1000, max(self.RTT_amt) * 1000, avgRTT))
        packetLoss = (1 - self.pack_data[1] / self.pack_data[0]) * 100
        print("PACKET LOSS RATE: " + str(int(packetLoss)) + "%\n")
    ...
    def __sendIcmpTraceRoute(self, host):
        ...
        while icmpType != 0:
            ...
            trace_packet.setTtl(seq_number)
            packetIdentifier = (os.getpid() & 0xffff)
            trace_packet_seq_num = seq_number

            trace_packet.buildPacket_echoRequest(packetIdentifier, trace_packet_seq_num)
            trace_packet.setIcmpTarget(host)
            # building the IP
            icmpType = trace_packet.sendEchoRequest()
...
def main():
    ...
    # Choose one of the following by uncommenting out the lines

    # icmpHelperPing.sendPing("209.233.126.254")
    icmpHelperPing.sendPing("oregonstate.edu")
    # icmpHelperPing.sendPing("www.amazon.it")
    # icmpHelperPing.sendPing("www.yahoo.co.jp")
    # icmpHelperPing.sendPing("gaia.cs.umass.edu")

	
    # icmpHelperPing.traceRoute("209.233.126.254")
    # icmpHelperPing.traceRoute("www.oregonstate.edu")
    # icmpHelperPing.traceRoute("www.amazon.it")        
    # icmpHelperPing.traceRoute("www.br.yahoo.com")     
